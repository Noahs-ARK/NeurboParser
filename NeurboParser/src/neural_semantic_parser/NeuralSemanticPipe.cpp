...
double NeuralSemanticPipe::NeuralTrainEpoch(const vector<int> &idxs, int epoch) {
    ...
    for (int i = 0; i < idxs.size(); i++) {
        ...
        Expression ex_loss = parser->BuildGraph(instance, parts, decoder_, scores,
...
void NeuralSemanticPipe::NeuralRun(double &unlabeled_F1, double &labeled_F1) {
    ...
    while (instance) {
        ...
        dynet::ComputationGraph cg;
        Expression ex_loss = parser->BuildGraph(formatted_instance, parts, decoder_, scores,
                                                gold_outputs, predicted_outputs,
                                                false, 0.0,
                                                form_count_,
                                                false, cg);
