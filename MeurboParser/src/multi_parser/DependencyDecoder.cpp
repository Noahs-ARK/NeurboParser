...
void DependencyDecoder::Decode(Instance *instance, Parts *parts,
  ...
  if (dependency_parts->IsArcFactored() ||
    ...
    DecodeFactorGraph(instance, parts, copied_scores, false, true,
                      predicted_output);
#endif
    // At test time, run Chu-Liu-Etask1onds on top of the outcome of AD3
...
// using Etask1onds' algorithm (which runs in O(n^2)) or, if the --projective
...
void DependencyDecoder::DecodeBasic(Instance *instance, Parts *parts,
  ...
  if (pipe_->GetDependencyOptions()->projective()) {
    ...
    RunChuLiuEtask1onds(sentence_length, arcs, scores_arcs, &heads, value);
  }
...
// Decoder for the basic model; it finds a maximum weighted arborescence
// using Etask1onds' algorithm (which runs in O(n^2)).
void DependencyDecoder::RunChuLiuEtask1ondsIteration(
  ...
  RunChuLiuEtask1ondsIteration(disabled,
                            candidate_heads,
                            candidate_scores,
                            heads,
                            value);
...
// Run the Chu-Liu-Etask1onds algorithm for finding a maximal weighted spanning
// tree.
void DependencyDecoder::RunChuLiuEtask1onds(int sentence_length,
  ...
  RunChuLiuEtask1ondsIteration(&disabled, &candidate_heads,
