...
enum {
    ...
    SEMANTICPART_CROSSFORM2NDORDER,
    SEMANTICPART_CROSSFORMLABELED2NDORDER,
    SEMANTICPART_CROSSFORM3RDORDER,
    SEMANTICPART_CROSSFORMLABELED3RDORDER,
...
class SemanticPartCrossForm2ndOrder : public Part {
public:
    SemanticPartCrossForm2ndOrder() { p_ = a_ = s_ = task1_r_ = task2_r_ = task3_r_ = -1; }

    SemanticPartCrossForm2ndOrder(int predicate, int argument, int sense, int task1_role, int task2_role, int task3_role) :
            p_(predicate), a_(argument), s_(sense), task1_r_(task1_role), task2_r_(task2_role), task3_r_(task3_role) {}

    virtual ~SemanticPartCrossForm2ndOrder() {}
...
public:
    ...
    int role(const string &formalism) {
...
public:
    int type() { return SEMANTICPART_CROSSFORM2NDORDER; }

private:
    int p_; // Index of the predicate.
    int a_; // Index of the argument.
    int s_; // Predicate sense.
    int task1_r_; // Role label.
    int task2_r_; // Role label.
    int task3_r_; // Role label.
};

// Part for a labeled arc linking a predicate and an argument word.
class SemanticPartCrossFormLabeled2ndOrder : public Part {
public:
    SemanticPartCrossFormLabeled2ndOrder() { p_ = a_ = s_ = task1_r_ = task2_r_ = task3_r_ = -1; }

    SemanticPartCrossFormLabeled2ndOrder(int predicate, int argument, int sense, int task1_role, int task2_role, int task3_role) :
            p_(predicate), a_(argument), s_(sense), task1_r_(task1_role), task2_r_(task2_role), task3_r_(task3_role) {}

    virtual ~SemanticPartCrossFormLabeled2ndOrder() {}
...
public:
    int type() { return SEMANTICPART_CROSSFORMLABELED2NDORDER; }
...
class SemanticPartCrossForm3rdOrder : public Part {
public:
    SemanticPartCrossForm3rdOrder() { p_ = a_ = s_ = task1_r_ = task2_r_ = task3_r_ = -1; }

    SemanticPartCrossForm3rdOrder(int predicate, int argument, int sense, int task1_role, int task2_role, int task3_role) :
            p_(predicate), a_(argument), s_(sense), task1_r_(task1_role), task2_r_(task2_role), task3_r_(task3_role) {}

    virtual ~SemanticPartCrossForm3rdOrder() {}
...
public:
    int type() { return SEMANTICPART_CROSSFORM3RDORDER; }
...
// Part for a labeled arc linking a predicate and an argument word.
class SemanticPartCrossFormLabeled3rdOrder : public Part {
public:
    SemanticPartCrossFormLabeled3rdOrder() { p_ = a_ = s_ = task1_r_ = task2_r_ = task3_r_ = -1; }

    SemanticPartCrossFormLabeled3rdOrder(int predicate, int argument, int sense, int task1_role, int task2_role, int task3_role) :
            p_(predicate), a_(argument), s_(sense), task1_r_(task1_role), task2_r_(task2_role), task3_r_(task3_role) {}

    virtual ~SemanticPartCrossFormLabeled3rdOrder() {}
...
public:
    int type() { return SEMANTICPART_CROSSFORMLABELED3RDORDER; }
...
public:
    ...
    Part *CreatePartCrossForm2ndOrder(int predicate, int argument, int sense, int task1_role, int task2_role, int task3_role) {
        return new SemanticPartCrossForm2ndOrder(predicate, argument, sense, task1_role, task2_role, task3_role);
    }

    Part *CreatePartCrossFormLabeled2ndOrder(int predicate, int argument, int sense, int task1_role, int task2_role, int task3_role) {
        return new SemanticPartCrossFormLabeled2ndOrder(predicate, argument, sense, task1_role, task2_role, task3_role);
    }

    Part *CreatePartCrossForm3rdOrder(int predicate, int argument, int sense, int task1_role, int task2_role, int task3_role) {
        return new SemanticPartCrossForm3rdOrder(predicate, argument, sense, task1_role, task2_role, task3_role);
    }

    Part *CreatePartCrossFormLabeled3rdOrder(int predicate, int argument, int sense, int task1_role, int task2_role, int task3_role) {
        return new SemanticPartCrossFormLabeled3rdOrder(predicate, argument, sense, task1_role, task2_role, task3_role);
    }
...
public:
    ...
    void SetOffsetCrossForm2ndOrder(int offset, int size) {
        SetOffset(SEMANTICPART_CROSSFORM2NDORDER, offset, size);
    };

    void SetOffsetCrossFormLabeled2ndOrder(int offset, int size) {
        SetOffset(SEMANTICPART_CROSSFORMLABELED2NDORDER, offset, size);
    };

    void SetOffsetCrossForm3rdOrder(int offset, int size) {
        SetOffset(SEMANTICPART_CROSSFORM3RDORDER, offset, size);
    };

    void SetOffsetCrossFormLabeled3rdOrder(int offset, int size) {
        SetOffset(SEMANTICPART_CROSSFORMLABELED3RDORDER, offset, size);
    };
    ...
    void GetOffsetCrossFormLabeled2ndOrder(int *offset, int *size) const {
        GetOffset(SEMANTICPART_CROSSFORMLABELED2NDORDER, offset, size);
    };

    void GetOffsetCrossForm2ndOrder(int *offset, int *size) const {
        GetOffset(SEMANTICPART_CROSSFORM2NDORDER, offset, size);
    };

    void GetOffsetCrossFormLabeled3rdOrder(int *offset, int *size) const {
        GetOffset(SEMANTICPART_CROSSFORMLABELED3RDORDER, offset, size);
    };

    void GetOffsetCrossForm3rdOrder(int *offset, int *size) const {
        GetOffset(SEMANTICPART_CROSSFORM3RDORDER, offset, size);
    };
