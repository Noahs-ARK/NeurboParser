...
void SemanticDecoder::DecodeCostAugmentedCrossForm(Instance *instance,
    ...
    int task1_offset_arcs, task1_num_arcs, task2_offset_arcs, task2_num_arcs, task3_offset_arcs, task3_num_arcs;
    task1_semantic_parts->GetOffsetLabeledArc(&task1_offset_arcs, &task1_num_arcs);
    task2_semantic_parts->GetOffsetLabeledArc(&task2_offset_arcs, &task2_num_arcs);
    task3_semantic_parts->GetOffsetLabeledArc(&task3_offset_arcs, &task3_num_arcs);
    double a = FLAGS_srl_train_cost_false_positives;
    double b = FLAGS_srl_train_cost_false_negatives;
    double q = 0.0;
    vector<double> task1_p(task1_num_arcs, 0.0);
    vector<double> task2_p(task2_num_arcs, 0.0);
    vector<double> task3_p(task3_num_arcs, 0.0);

    vector<double> task1_scores_cost = task1_scores;
    vector<double> task2_scores_cost = task2_scores;
    vector<double> task3_scores_cost = task3_scores;
    for (int r = 0; r < task1_num_arcs; ++r) {
        task1_p[r] = a - (a + b) * task1_gold_output[task1_offset_arcs + r];
        task1_scores_cost[task1_offset_arcs + r] += task1_p[r];
        q += b * task1_gold_output[task1_offset_arcs + r];
    }
    for (int r = 0; r < task2_num_arcs; ++r) {
        task2_p[r] = a - (a + b) * task2_gold_output[task2_offset_arcs + r];
        task2_scores_cost[task2_offset_arcs + r] += task2_p[r];
        q += b * task2_gold_output[task2_offset_arcs + r];
    }
    a = 0.3;
    b = 0.7;
    for (int r = 0; r < task3_num_arcs; ++r) {
        task3_p[r] = a - (a + b) * task3_gold_output[task3_offset_arcs + r];
        task3_scores_cost[task3_offset_arcs + r] += task3_p[r];
        q += b * task3_gold_output[task3_offset_arcs + r];
    }
    DecodeCrossForm(instance,
                    task1_parts, task1_scores_cost, task1_predicted_output,
                    task2_parts, task2_scores_cost, task2_predicted_output,
                    task3_parts, task3_scores_cost, task3_predicted_output,
                    global_parts, global_scores, global_predicted_output);
    *cost = q;
    for (int r = 0; r < task1_num_arcs; ++r) {
        *cost += task1_p[r] * (*task1_predicted_output)[task1_offset_arcs + r];
    }
    for (int r = 0; r < task2_num_arcs; ++r) {
        *cost += task2_p[r] * (*task2_predicted_output)[task2_offset_arcs + r];
    }
    for (int r = 0; r < task3_num_arcs; ++r) {
        *cost += task3_p[r] * (*task3_predicted_output)[task3_offset_arcs + r];
    }
    *loss = *cost;
//    for (int r = 0; r < task1_parts->size(); ++r) {
//        *loss += task1_scores[r] * ((*task1_predicted_output)[r] - task1_gold_output[r]);
//    }
//    for (int r = 0; r < task2_parts->size(); ++r) {
//        *loss += task2_scores[r] * ((*task2_predicted_output)[r] - task2_gold_output[r]);
//    }
//    for (int r = 0; r < task3_parts->size(); ++r) {
//        *loss += task3_scores[r] * ((*task3_predicted_output)[r] - task3_gold_output[r]);
//    }
}
...
//    LOG(INFO) << "relax: "<< false<<endl;
    DecodeFactorGraph(instance, parts, copied_scores, true, true,
                      predicted_output);
...
void SemanticDecoder::DecodeCrossForm(Instance *instance,
                                      Parts *task1_parts, const vector<double> &task1_scores,
                                      vector<double> *task1_predicted_output,
                                      Parts *task2_parts, const vector<double> &task2_scores,
                                      vector<double> *task2_predicted_output,
                                      ...
                                      vector<double> *global_predicted_output) {
    SemanticParts *task1_semantic_parts = static_cast<SemanticParts *>(task1_parts);
    vector<double> task1_copied_scores(task1_scores);
    vector<int> task1_best_labeled_parts;
    ...
    task1_predicted_output->clear();
    task1_predicted_output->resize(task1_parts->size(), 0.0);

    SemanticParts *task2_semantic_parts = static_cast<SemanticParts *>(task2_parts);
    vector<double> task2_copied_scores(task2_scores);
    vector<int> task2_best_labeled_parts;
    ...
    task2_predicted_output->clear();
    task2_predicted_output->resize(task2_parts->size(), 0.0);

    SemanticParts *task3_semantic_parts = static_cast<SemanticParts *>(task3_parts);
    vector<double> task3_copied_scores(task3_scores);
    vector<int> task3_best_labeled_parts;
    ...
    task3_predicted_output->clear();
    task3_predicted_output->resize(task3_parts->size(), 0.0);

    DecodeFactorGraphCrossForm(instance,
                               task1_parts, task1_copied_scores, task1_predicted_output,
                               task2_parts, task2_copied_scores, task2_predicted_output,
                               task3_parts, task3_copied_scores, task3_predicted_output,
                               global_parts, global_scores, global_predicted_output,
                               true, true);
//    for (int r = 0; r < task3_predicted_output->size(); ++ r) {
//        cout <<  (*task3_predicted_output)[r] <<" "
//             << task3_scores[r] << " "<<(*task3_parts)[r] ->type()<<endl;
//    }
//    cout <<endl<<endl;
    ...
    if (!pipe_->GetSemanticOptions()->train()) {
        double threshold = 0.5;
        for (int r = 0; r < task1_num_arcs; ++r) {
            task1_copied_scores[task1_offset_arcs + r] = 0.0;
        }
        for (int r = 0; r < task1_num_predicate_parts; ++r) {
            task1_copied_scores[task1_offset_predicate_parts + r] = 0.0;
        }
        for (int r = 0; r < task1_num_labeled_arcs; ++r) {
            task1_copied_scores[task1_offset_labeled_arcs + r] =
                    (*task1_predicted_output)[task1_offset_labeled_arcs + r] - threshold;
        }

        DecodeLabels(instance, task1_parts, task1_copied_scores, &task1_best_labeled_parts);
        for (int r = 0; r < task1_best_labeled_parts.size(); ++r) {
            // Sum the "labeled" scores to the (eventually) already existing
            // "unlabeled" scores.
            task1_copied_scores[task1_offset_arcs + r] += task1_copied_scores[task1_best_labeled_parts[r]];
        }

        double value;
        task1_predicted_output->assign(task1_parts->size(), 0.0);
        DecodeBasic(instance, task1_parts, task1_copied_scores, task1_predicted_output, &value);

        // Write the components of the predicted output that
        // correspond to the labeled parts.
        for (int r = 0; r < task1_num_arcs; ++r) {
            CHECK_GE(task1_best_labeled_parts[r], task1_offset_arcs + task1_num_arcs);
            (*task1_predicted_output)[task1_best_labeled_parts[r]] =
                    (*task1_predicted_output)[task1_offset_arcs + r];
        }

        for (int r = 0; r < task2_num_arcs; ++r) {
            task2_copied_scores[task2_offset_arcs + r] = 0.0;
        }
        for (int r = 0; r < task2_num_predicate_parts; ++r) {
            task2_copied_scores[task2_offset_predicate_parts + r] = 0.0;
        }
        for (int r = 0; r < task2_num_labeled_arcs; ++r) {
            task2_copied_scores[task2_offset_labeled_arcs + r] =
                    (*task2_predicted_output)[task2_offset_labeled_arcs + r] - threshold;
        }

        DecodeLabels(instance, task2_parts, task2_copied_scores, &task2_best_labeled_parts);
        for (int r = 0; r < task2_best_labeled_parts.size(); ++r) {
            // Sum the "labeled" scores to the (eventually) already existing
            // "unlabeled" scores.
            task2_copied_scores[task2_offset_arcs + r] += task2_copied_scores[task2_best_labeled_parts[r]];
        }

        task2_predicted_output->assign(task2_parts->size(), 0.0);
        DecodeBasic(instance, task2_parts, task2_copied_scores, task2_predicted_output, &value);

        // Write the components of the predicted output that
        // correspond to the labeled parts.
        for (int r = 0; r < task2_num_arcs; ++r) {
            CHECK_GE(task2_best_labeled_parts[r], task2_offset_arcs + task2_num_arcs);
            (*task2_predicted_output)[task2_best_labeled_parts[r]] =
                    (*task2_predicted_output)[task2_offset_arcs + r];
        }

        for (int r = 0; r < task3_num_arcs; ++r) {
            task3_copied_scores[task3_offset_arcs + r] = 0.0;
        }
        for (int r = 0; r < task3_num_predicate_parts; ++r) {
            task3_copied_scores[task3_offset_predicate_parts + r] = 0.0;
        }
        for (int r = 0; r < task3_num_labeled_arcs; ++r) {
            task3_copied_scores[task3_offset_labeled_arcs + r] =
                    (*task3_predicted_output)[task3_offset_labeled_arcs + r] - threshold;
        }

        DecodeLabels(instance, task3_parts, task3_copied_scores, &task3_best_labeled_parts);
        for (int r = 0; r < task3_best_labeled_parts.size(); ++r) {
            // Sum the "labeled" scores to the (eventually) already existing
            // "unlabeled" scores.
            task3_copied_scores[task3_offset_arcs + r] += task3_copied_scores[task3_best_labeled_parts[r]];
        }

        task3_predicted_output->assign(task3_parts->size(), 0.0);
        DecodeBasic(instance, task3_parts, task3_copied_scores, task3_predicted_output, &value);

        // Write the components of the predicted output that
        // correspond to the labeled parts.
        for (int r = 0; r < task3_num_arcs; ++r) {
            CHECK_GE(task3_best_labeled_parts[r], task3_offset_arcs + task3_num_arcs);
            (*task3_predicted_output)[task3_best_labeled_parts[r]] =
                    (*task3_predicted_output)[task3_offset_arcs + r];
        }
    }
...
void SemanticDecoder::DecodeFactorGraphCrossForm(Instance *instance,
                                                 Parts *task1_parts, const vector<double> &task1_scores,
                                                 vector<double> *task1_predicted_output,
                                                 Parts *task2_parts, const vector<double> &task2_scores,
                                                 vector<double> *task2_predicted_output,
    ...
    SemanticParts *global_semantic_parts = static_cast<SemanticParts *>(global_parts);
    int global_offset_2ndorder, global_num_2ndorder;
    global_semantic_parts->GetOffsetCrossForm2ndOrder(&global_offset_2ndorder, &global_num_2ndorder);

    int global_offset_labeled_2ndorder, global_num_labeled_2ndorder;
    global_semantic_parts->GetOffsetCrossFormLabeled2ndOrder(&global_offset_labeled_2ndorder,
                                                             &global_num_labeled_2ndorder);
    int global_offset_3rdorder, global_num_3rdorder;
    global_semantic_parts->GetOffsetCrossForm3rdOrder(&global_offset_3rdorder, &global_num_3rdorder);

    int global_offset_labeled_3rdorder, global_num_labeled_3rdorder;
    global_semantic_parts->GetOffsetCrossFormLabeled3rdOrder(&global_offset_labeled_3rdorder,
                                                             &global_num_labeled_3rdorder);
    vector<AD3::BinaryVariable *> variables;
    vector<int> part_indices_;
    vector<int> additional_part_indices;
    vector<int> factor_part_indices_;
    ...
    factor_graph->SetVerbosity(verbosity);
    int task1_offset_predicate_variables = variables.size();
    for (int r = 0; r < task1_num_predicate_parts; ++r) {
        AD3::BinaryVariable *variable = factor_graph->CreateBinaryVariable();
        variable->SetLogPotential(task1_scores[task1_offset_predicate_parts + r]);
        variables.push_back(variable);
        part_indices_.push_back(task1_offset_predicate_parts + r);
    }
    int task1_offset_arc_variables = variables.size();
    for (int r = 0; r < task1_num_arcs; ++r) {
        AD3::BinaryVariable *variable = factor_graph->CreateBinaryVariable();
        variable->SetLogPotential(task1_scores[task1_offset_arcs + r]);
        variables.push_back(variable);
        part_indices_.push_back(task1_offset_arcs + r);
    }
    int task1_offset_labeled_arc_variables = variables.size();
    if (labeled_decoding) {
        for (int r = 0; r < task1_num_labeled_arcs; ++r) {
            AD3::BinaryVariable *variable = factor_graph->CreateBinaryVariable();
            variable->SetLogPotential(task1_scores[task1_offset_labeled_arcs + r]);
            variables.push_back(variable);
            part_indices_.push_back(task1_offset_labeled_arcs + r);
        }
    }
    int task2_offset_predicate_variables = variables.size();
    for (int r = 0; r < task2_num_predicate_parts; ++r) {
        AD3::BinaryVariable *variable = factor_graph->CreateBinaryVariable();
        variable->SetLogPotential(task2_scores[task2_offset_predicate_parts + r]);
        variables.push_back(variable);
        part_indices_.push_back(task2_offset_predicate_parts + r);
    }
    int task2_offset_arc_variables = variables.size();
    for (int r = 0; r < task2_num_arcs; ++r) {
        AD3::BinaryVariable *variable = factor_graph->CreateBinaryVariable();
        variable->SetLogPotential(task2_scores[task2_offset_arcs + r]);
        variables.push_back(variable);
        part_indices_.push_back(task2_offset_arcs + r);
    }
    int task2_offset_labeled_arc_variables = variables.size();
    if (labeled_decoding) {
        for (int r = 0; r < task2_num_labeled_arcs; ++r) {
            AD3::BinaryVariable *variable = factor_graph->CreateBinaryVariable();
            variable->SetLogPotential(task2_scores[task2_offset_labeled_arcs + r]);
            variables.push_back(variable);
            part_indices_.push_back(task2_offset_labeled_arcs + r);
        }
    }
    int task3_offset_predicate_variables = variables.size();
    for (int r = 0; r < task3_num_predicate_parts; ++r) {
        AD3::BinaryVariable *variable = factor_graph->CreateBinaryVariable();
        variable->SetLogPotential(task3_scores[task3_offset_predicate_parts + r]);
        variables.push_back(variable);
        part_indices_.push_back(task3_offset_predicate_parts + r);
    }
    int task3_offset_arc_variables = variables.size();
    for (int r = 0; r < task3_num_arcs; ++r) {
        AD3::BinaryVariable *variable = factor_graph->CreateBinaryVariable();
        variable->SetLogPotential(task3_scores[task3_offset_arcs + r]);
        variables.push_back(variable);
        part_indices_.push_back(task3_offset_arcs + r);
    }
    int task3_offset_labeled_arc_variables = variables.size();
    if (labeled_decoding) {
        for (int r = 0; r < task3_num_labeled_arcs; ++r) {
            AD3::BinaryVariable *variable = factor_graph->CreateBinaryVariable();
            variable->SetLogPotential(task3_scores[task3_offset_labeled_arcs + r]);
            variables.push_back(variable);
            part_indices_.push_back(task3_offset_labeled_arcs + r);
        }
    }

    vector<AD3::BinaryVariable *> local_variables;
    int task1_offset = 0;
    vector<SemanticPartPredicate *> task1_predicate_parts;
    vector<SemanticPartArc *> task1_arcs;
    int task2_offset = 0;
    vector<SemanticPartPredicate *> task2_predicate_parts;
    vector<SemanticPartArc *> task2_arcs;
    int task3_offset = 0;
    vector<SemanticPartPredicate *> task3_predicate_parts;
    vector<SemanticPartArc *> task3_arcs;
    task1_offset = local_variables.size();
    for (int r = 0; r < task1_num_predicate_parts; ++r) {
        local_variables.push_back(variables[task1_offset_predicate_variables + r]);
        SemanticPartPredicate *predicate = static_cast<SemanticPartPredicate *>((*task1_parts)[
                task1_offset_predicate_parts +
                r]);
        task1_predicate_parts.push_back(predicate);
    }
    for (int r = 0; r < task1_num_arcs; ++r) {
        local_variables.push_back(variables[task1_offset_arc_variables + r]);
        SemanticPartArc *arc = static_cast<SemanticPartArc *>((*task1_parts)[task1_offset_arcs + r]);
        task1_arcs.push_back(arc);
    }
    task2_offset = local_variables.size();
    for (int r = 0; r < task2_num_predicate_parts; ++r) {
        local_variables.push_back(variables[task2_offset_predicate_variables + r]);
        SemanticPartPredicate *predicate = static_cast<SemanticPartPredicate *>((*task2_parts)[
                task2_offset_predicate_parts +
                r]);
        task2_predicate_parts.push_back(predicate);
    }
    for (int r = 0; r < task2_num_arcs; ++r) {
        local_variables.push_back(variables[task2_offset_arc_variables + r]);
        SemanticPartArc *arc = static_cast<SemanticPartArc *>((*task2_parts)[task2_offset_arcs + r]);
        task2_arcs.push_back(arc);
    }
    task3_offset = local_variables.size();
    for (int r = 0; r < task3_num_predicate_parts; ++r) {
        local_variables.push_back(variables[task3_offset_predicate_variables + r]);
        SemanticPartPredicate *predicate = static_cast<SemanticPartPredicate *>((*task3_parts)[
                task3_offset_predicate_parts +
                r]);
        task3_predicate_parts.push_back(predicate);
    }
    for (int r = 0; r < task3_num_arcs; ++r) {
        local_variables.push_back(variables[task3_offset_arc_variables + r]);
        SemanticPartArc *arc = static_cast<SemanticPartArc *>((*task3_parts)[task3_offset_arcs + r]);
        task3_arcs.push_back(arc);
    }


    AD3::FactorCrossFormSemanticGraph *factor = new AD3::FactorCrossFormSemanticGraph;
    factor->Initialize(sentence->size(),
                       task1_offset, task1_predicate_parts, task1_arcs,
                       task2_offset, task2_predicate_parts, task2_arcs,
                       task3_offset, task3_predicate_parts, task3_arcs,
                       this);
    factor_graph->DeclareFactor(factor, local_variables, true);
    factor_part_indices_.push_back(-1);
    ...
    if (labeled_decoding) {
        ...
        for (int r = 0; r < task1_num_arcs; ++r) {
            const vector<int> &index_labeled_parts =
                    task1_semantic_parts->GetLabeledParts(task1_offset_arcs + r);
            vector<AD3::BinaryVariable *>
                    local_variables(index_labeled_parts.size() + 1);
            for (int k = 0; k < index_labeled_parts.size(); ++k) {
                int index_part = index_labeled_parts[k];
                CHECK_GE(index_part, 0);
                CHECK_LT(task1_offset_labeled_arc_variables + index_part - task1_offset_labeled_arcs,
                         variables.size());
                local_variables[k] = variables[task1_offset_labeled_arc_variables +
                                               index_part - task1_offset_labeled_arcs];
            }
            CHECK_GE(task1_offset_arc_variables + r, 0);
            CHECK_LT(task1_offset_arc_variables + r, variables.size());
            local_variables[index_labeled_parts.size()] =
                    variables[task1_offset_arc_variables + r];
            factor_graph->CreateFactorXOROUT(local_variables);
            factor_part_indices_.push_back(-1);
        }
        /* task1 end */
        ...
        for (int r = 0; r < task2_num_arcs; ++r) {
            const vector<int> &index_labeled_parts =
                    task2_semantic_parts->GetLabeledParts(task2_offset_arcs + r);
            vector<AD3::BinaryVariable *>
                    local_variables(index_labeled_parts.size() + 1);
            for (int k = 0; k < index_labeled_parts.size(); ++k) {
                int index_part = index_labeled_parts[k];
                CHECK_GE(index_part, 0);
                CHECK_LT(task2_offset_labeled_arc_variables + index_part - task2_offset_labeled_arcs,
                         variables.size());
                local_variables[k] = variables[task2_offset_labeled_arc_variables +
                                               index_part - task2_offset_labeled_arcs];
            }
            CHECK_GE(task2_offset_arc_variables + r, 0);
            CHECK_LT(task2_offset_arc_variables + r, variables.size());
            local_variables[index_labeled_parts.size()] =
                    variables[task2_offset_arc_variables + r];
            factor_graph->CreateFactorXOROUT(local_variables);
            factor_part_indices_.push_back(-1);
        }
        /* task2 end */
        ...
        for (int r = 0; r < task3_num_arcs; ++r) {
            const vector<int> &index_labeled_parts =
                    task3_semantic_parts->GetLabeledParts(task3_offset_arcs + r);
            vector<AD3::BinaryVariable *>
                    local_variables(index_labeled_parts.size() + 1);
            for (int k = 0; k < index_labeled_parts.size(); ++k) {
                int index_part = index_labeled_parts[k];
                CHECK_GE(index_part, 0);
                CHECK_LT(task3_offset_labeled_arc_variables + index_part - task3_offset_labeled_arcs,
                         variables.size());
                local_variables[k] = variables[task3_offset_labeled_arc_variables +
                                               index_part - task3_offset_labeled_arcs];
            }
            CHECK_GE(task3_offset_arc_variables + r, 0);
            CHECK_LT(task3_offset_arc_variables + r, variables.size());
            local_variables[index_labeled_parts.size()] =
                    variables[task3_offset_arc_variables + r];
            factor_graph->CreateFactorXOROUT(local_variables);
            factor_part_indices_.push_back(-1);
        }
        /* task3 end */
        ...
        if (pipe_->GetSemanticOptions()->deterministic_labels()) {
            ...
            for (int r = 0; r < task1_num_labeled_arcs; ++r) {
                SemanticPartLabeledArc *labeled_arc =
                        static_cast<SemanticPartLabeledArc *>(
                                (*task1_semantic_parts)[task1_offset_labeled_arcs + r]);
                int p = labeled_arc->predicate();
                int l = labeled_arc->role();
                // Skip if role l is not deterministic.
                if (!pipe_->GetSemanticDictionary("task1")->IsRoleDeterministic(l)) continue;
                if (labeled_arcs_by_predicate_role[p].size() <= l) {
                    labeled_arcs_by_predicate_role[p].resize(l + 1);
                }
                labeled_arcs_by_predicate_role[p][l].push_back(r);
            ...
            for (int p = 0; p < labeled_arcs_by_predicate_role.size(); ++p) {
                for (int l = 0; l < labeled_arcs_by_predicate_role[p].size(); ++l) {
                    ...
                    for (int k = 0; k < labeled_arcs_by_predicate_role[p][l].size();
                         ++k) {
                        int r = labeled_arcs_by_predicate_role[p][l][k];
                        local_variables[k] = variables[task1_offset_labeled_arc_variables + r];
                    }
                    factor_graph->CreateFactorAtMostOne(local_variables);
                    factor_part_indices_.push_back(-1);
                }
            ...
            /* task1 end */
            /* task2 begin */
            labeled_arcs_by_predicate_role.clear();
            labeled_arcs_by_predicate_role.resize(sentence_length);
            for (int r = 0; r < task2_num_labeled_arcs; ++r) {
                SemanticPartLabeledArc *labeled_arc =
                        static_cast<SemanticPartLabeledArc *>(
                                (*task2_semantic_parts)[task2_offset_labeled_arcs + r]);
                int p = labeled_arc->predicate();
                int l = labeled_arc->role();
                // Skip if role l is not deterministic.
                if (!pipe_->GetSemanticDictionary("task2")->IsRoleDeterministic(l)) continue;
                if (labeled_arcs_by_predicate_role[p].size() <= l) {
                    labeled_arcs_by_predicate_role[p].resize(l + 1);
                }
                labeled_arcs_by_predicate_role[p][l].push_back(r);
            ...
            for (int p = 0; p < labeled_arcs_by_predicate_role.size(); ++p) {
                for (int l = 0; l < labeled_arcs_by_predicate_role[p].size(); ++l) {
                    ...
                    for (int k = 0; k < labeled_arcs_by_predicate_role[p][l].size();
                         ++k) {
                        int r = labeled_arcs_by_predicate_role[p][l][k];
                        local_variables[k] = variables[task2_offset_labeled_arc_variables + r];
                    }
                    factor_graph->CreateFactorAtMostOne(local_variables);
                    factor_part_indices_.push_back(-1);
                }
            ...
            for (int r = 0; r < task3_num_labeled_arcs; ++r) {
                SemanticPartLabeledArc *labeled_arc =
                        static_cast<SemanticPartLabeledArc *>(
                                (*task3_semantic_parts)[task3_offset_labeled_arcs + r]);
                int p = labeled_arc->predicate();
                int l = labeled_arc->role();
                // Skip if role l is not deterministic.
                if (!pipe_->GetSemanticDictionary("task3")->IsRoleDeterministic(l)) continue;
                if (labeled_arcs_by_predicate_role[p].size() <= l) {
                    labeled_arcs_by_predicate_role[p].resize(l + 1);
                }
                labeled_arcs_by_predicate_role[p][l].push_back(r);
            ...
            for (int p = 0; p < labeled_arcs_by_predicate_role.size(); ++p) {
                for (int l = 0; l < labeled_arcs_by_predicate_role[p].size(); ++l) {
                    ...
                    for (int k = 0; k < labeled_arcs_by_predicate_role[p][l].size();
                         ++k) {
                        int r = labeled_arcs_by_predicate_role[p][l][k];
                        local_variables[k] = variables[task3_offset_labeled_arc_variables + r];
                    }
                    factor_graph->CreateFactorAtMostOne(local_variables);
                    factor_part_indices_.push_back(-1);
                }
    ...
    for (int r = 0; r < global_num_2ndorder; ++r) {
        SemanticPartCrossForm2ndOrder *part = static_cast<SemanticPartCrossForm2ndOrder *>(
                (*global_semantic_parts)[global_offset_2ndorder + r]);

        int task1_role = part->role("task1");
        int task2_role = part->role("task2");
        int task3_role = part->role("task3");

        if (task1_role > 0 && task2_role > 0 && task3_role < 0) {
            int task1_idx = task1_semantic_parts->FindArc(part->predicate(),
                                                          part->argument(),
                                                          part->sense());
            int task2_idx = task2_semantic_parts->FindArc(part->predicate(),
                                                          part->argument(),
                                                          part->sense());
            CHECK_GE(task1_idx, 0);
            CHECK_GE(task1_offset_arc_variables + task1_idx - task1_offset_arcs, 0);
            CHECK_LT(task1_offset_arc_variables + task1_idx - task1_offset_arcs, variables.size());

            CHECK_GE(task2_idx, 0);
            CHECK_GE(task2_offset_arc_variables + task2_idx - task2_offset_arcs, 0);
            CHECK_LT(task2_offset_arc_variables + task2_idx - task2_offset_arcs, variables.size());

            vector<AD3::BinaryVariable *> local_variables;
            local_variables.push_back(variables[task1_offset_arc_variables + task1_idx - task1_offset_arcs]);
            local_variables.push_back(variables[task2_offset_arc_variables + task2_idx - task2_offset_arcs]);

            factor_graph->CreateFactorPAIR(local_variables, global_scores[global_offset_2ndorder + r]);
            additional_part_indices.push_back(global_offset_2ndorder + r);
            factor_part_indices_.push_back(global_offset_2ndorder + r);
        } else if (task1_role > 0 && task2_role < 0 && task3_role > 0) {
            int task1_idx = task1_semantic_parts->FindArc(part->predicate(),
                                                          part->argument(),
                                                          part->sense());
            int task3_idx = task3_semantic_parts->FindArc(part->predicate(),
                                                          part->argument(),
                                                          part->sense());
            CHECK_GE(task1_idx, 0);
            CHECK_GE(task1_offset_arc_variables + task1_idx - task1_offset_arcs, 0);
            ...
            CHECK_GE(task3_idx, 0);
            CHECK_GE(task3_offset_arc_variables + task3_idx - task3_offset_arcs, 0);
            CHECK_LT(task3_offset_arc_variables + task3_idx - task3_offset_arcs, variables.size());

            vector<AD3::BinaryVariable *> local_variables;
            local_variables.push_back(variables[task1_offset_arc_variables + task1_idx - task1_offset_arcs]);
            local_variables.push_back(variables[task3_offset_arc_variables + task3_idx - task3_offset_arcs]);
    ...
    int global_offset_3rdorder_variables = variables.size();
    for (int r = 0; r < global_num_3rdorder; ++r) {
        SemanticPartCrossForm3rdOrder *part = static_cast<SemanticPartCrossForm3rdOrder *>(
                (*global_semantic_parts)[global_offset_3rdorder + r]);
        ...
        if (task1_role > 0 && task2_role > 0 && task3_role > 0) {
            ...
            AD3::BinaryVariable *variable = factor_graph->CreateBinaryVariable();
            variable->SetLogPotential(global_scores[global_offset_3rdorder + r]);
            variables.push_back(variable);
            local_variables.push_back(variable);
            factor_graph->CreateFactorANDOUT(local_variables);
            part_indices_.push_back(global_offset_3rdorder + r);
            factor_part_indices_.push_back(global_offset_3rdorder + r);
        } else {
            CHECK(false) << "Cross form arc error. Giving up.";
        }
    }
    ...
    for (int r = 0; r < global_num_labeled_2ndorder; ++r) {
        SemanticPartCrossFormLabeled2ndOrder *part = static_cast<SemanticPartCrossFormLabeled2ndOrder *>(
                (*global_semantic_parts)[global_offset_labeled_2ndorder + r]);
        ...
        if (task1_role >= 0 && task2_role >= 0 && task3_role < 0) {
            int task1_idx = task1_semantic_parts->FindLabeledArc(part->predicate(),
                                                                 part->argument(),
                                                                 part->sense(),
                                                                 task1_role);
            int task2_idx = task2_semantic_parts->FindLabeledArc(part->predicate(),
                                                                 part->argument(),
                                                                 part->sense(),
                                                                 task2_role);
            CHECK_GE(task1_idx, 0);
            CHECK_GE(task1_offset_labeled_arc_variables + task1_idx - task1_offset_labeled_arcs, 0);
            CHECK_LT(task1_offset_labeled_arc_variables + task1_idx - task1_offset_labeled_arcs, variables.size());

            CHECK_GE(task2_idx, 0);
            CHECK_GE(task2_offset_labeled_arc_variables + task2_idx - task2_offset_labeled_arcs, 0);
            CHECK_LT(task2_offset_labeled_arc_variables + task2_idx - task2_offset_labeled_arcs, variables.size());

            vector<AD3::BinaryVariable *> local_variables;
            local_variables.push_back(
                    variables[task1_offset_labeled_arc_variables + task1_idx - task1_offset_labeled_arcs]);
            local_variables.push_back(
                    variables[task2_offset_labeled_arc_variables + task2_idx - task2_offset_labeled_arcs]);
            factor_graph->CreateFactorPAIR(local_variables, global_scores[global_offset_labeled_2ndorder + r]);
            additional_part_indices.push_back(global_offset_labeled_2ndorder + r);
            factor_part_indices_.push_back(global_offset_labeled_2ndorder + r);
        } else if (task1_role >= 0 && task2_role < 0 && task3_role >= 0) {
            int task1_idx = task1_semantic_parts->FindLabeledArc(part->predicate(),
                                                                 part->argument(),
                                                                 part->sense(),
                                                                 task1_role);
            int task3_idx = task3_semantic_parts->FindLabeledArc(part->predicate(),
                                                                 part->argument(),
                                                                 part->sense(),
                                                                 task3_role);
            CHECK_GE(task1_idx, 0);
            ...
            CHECK_GE(task3_idx, 0);
            CHECK_GE(task3_offset_labeled_arc_variables + task3_idx - task3_offset_labeled_arcs, 0);
            CHECK_LT(task3_offset_labeled_arc_variables + task3_idx - task3_offset_labeled_arcs, variables.size());

            vector<AD3::BinaryVariable *> local_variables;
            local_variables.push_back(
                    variables[task1_offset_labeled_arc_variables + task1_idx - task1_offset_labeled_arcs]);
            local_variables.push_back(
                    variables[task3_offset_labeled_arc_variables + task3_idx - task3_offset_labeled_arcs]);
            factor_graph->CreateFactorPAIR(local_variables, global_scores[global_offset_labeled_2ndorder + r]);
            additional_part_indices.push_back(global_offset_labeled_2ndorder + r);
            factor_part_indices_.push_back(global_offset_labeled_2ndorder + r);
        } else if (task1_role < 0 && task2_role >= 0 && task3_role >= 0) {
            ...
            LOG(INFO) << "Cross form labeled arc error. Giving up.";
            CHECK(false);
        }
    }

    int global_offset_labeled_3rdorder_variables = variables.size();
    for (int r = 0; r < global_num_labeled_3rdorder; ++r) {
        SemanticPartCrossFormLabeled3rdOrder *part = static_cast<SemanticPartCrossFormLabeled3rdOrder *>(
                (*global_semantic_parts)[global_offset_labeled_3rdorder + r]);
        ...
        if (task1_role >= 0 && task2_role >= 0 && task3_role >= 0) {
            ...
            AD3::BinaryVariable *variable = factor_graph->CreateBinaryVariable();
            variable->SetLogPotential(global_scores[global_offset_labeled_3rdorder + r]);
            variables.push_back(variable);
            local_variables.push_back(variable);
            factor_graph->CreateFactorANDOUT(local_variables);
            part_indices_.push_back(global_offset_labeled_3rdorder + r);
            factor_part_indices_.push_back(global_offset_labeled_3rdorder + r);
        } else {
    ...
    CHECK_EQ(variables.size(), part_indices_.size());
    CHECK_EQ(factor_graph->GetNumFactors(), factor_part_indices_.size());
    CHECK_EQ(additional_part_indices.size(), global_num_2ndorder + global_num_labeled_2ndorder);
    ...
    *value = 0.0;
    ...
    task2_predicted_output->assign(task2_parts->size(), 0.0);
    task3_predicted_output->assign(task3_parts->size(), 0.0);
    global_predicted_output->assign(global_parts->size(), 0.0);
    for (int r = 0; r < task1_num_predicate_parts; ++r) {
        int i = part_indices_[r + task1_offset_predicate_variables];
        CHECK_EQ(i, r + task1_offset_predicate_parts);
        (*task1_predicted_output)[i] = posteriors[r + task1_offset_predicate_variables];
        *value += (*task1_predicted_output)[i] * task1_scores[i];
    }
    for (int r = 0; r < task1_num_arcs; ++r) {
        int i = part_indices_[r + task1_offset_arc_variables];
        CHECK_EQ(i, r + task1_offset_arcs);
        (*task1_predicted_output)[i] = posteriors[r + task1_offset_arc_variables];
        *value += (*task1_predicted_output)[i] * task1_scores[i];
    }
    for (int r = 0; r < task1_num_labeled_arcs; ++r) {
        int i = part_indices_[r + task1_offset_labeled_arc_variables];
        CHECK_EQ(i, r + task1_offset_labeled_arcs);
        (*task1_predicted_output)[i] = posteriors[r + task1_offset_labeled_arc_variables];
        *value += (*task1_predicted_output)[i] * task1_scores[i];
    }

    for (int r = 0; r < task2_num_predicate_parts; ++r) {
        int i = part_indices_[r + task2_offset_predicate_variables];
        CHECK_EQ(i, r + task2_offset_predicate_parts);
        (*task2_predicted_output)[i] = posteriors[r + task2_offset_predicate_variables];
        *value += (*task2_predicted_output)[i] * task2_scores[i];
    }
    for (int r = 0; r < task2_num_arcs; ++r) {
        int i = part_indices_[r + task2_offset_arc_variables];
        CHECK_EQ(i, r + task2_offset_arcs);
        (*task2_predicted_output)[i] = posteriors[r + task2_offset_arc_variables];
        *value += (*task2_predicted_output)[i] * task2_scores[i];
    }
    for (int r = 0; r < task2_num_labeled_arcs; ++r) {
        int i = part_indices_[r + task2_offset_labeled_arc_variables];
        CHECK_EQ(i, r + task2_offset_labeled_arcs);
        (*task2_predicted_output)[i] = posteriors[r + task2_offset_labeled_arc_variables];
        *value += (*task2_predicted_output)[i] * task2_scores[i];
    }

    for (int r = 0; r < task3_num_predicate_parts; ++r) {
        int i = part_indices_[r + task3_offset_predicate_variables];
        CHECK_EQ(i, r + task3_offset_predicate_parts);
        (*task3_predicted_output)[i] = posteriors[r + task3_offset_predicate_variables];
        *value += (*task3_predicted_output)[i] * task3_scores[i];
    }
    for (int r = 0; r < task3_num_arcs; ++r) {
        int i = part_indices_[r + task3_offset_arc_variables];
        CHECK_EQ(i, r + task3_offset_arcs);
        (*task3_predicted_output)[i] = posteriors[r + task3_offset_arc_variables];
        *value += (*task3_predicted_output)[i] * task3_scores[i];
    }
    for (int r = 0; r < task3_num_labeled_arcs; ++r) {
        int i = part_indices_[r + task3_offset_labeled_arc_variables];
        CHECK_EQ(i, r + task3_offset_labeled_arcs);
        (*task3_predicted_output)[i] = posteriors[r + task3_offset_labeled_arc_variables];
        *value += (*task3_predicted_output)[i] * task3_scores[i];
    }
    for (int r = 0; r < additional_part_indices.size(); ++r) {
        int i = additional_part_indices[r];
        (*global_predicted_output)[i] = additional_posteriors[r];
        *value += (*global_predicted_output)[i] * global_scores[i];
    }
    for (int r = 0; r < global_num_3rdorder; ++r) {
        int i = part_indices_[r + global_offset_3rdorder_variables];
        CHECK_EQ(i, r + global_offset_3rdorder);
        (*global_predicted_output)[i] = posteriors[r + global_offset_3rdorder_variables];
        *value += (*global_predicted_output)[i] * global_scores[i];
    }
    for (int r = 0; r < global_num_labeled_3rdorder; ++r) {
        int i = part_indices_[r + global_offset_labeled_3rdorder_variables];
        CHECK_EQ(i, r + global_offset_labeled_3rdorder);
        (*global_predicted_output)[i] = posteriors[r + global_offset_labeled_3rdorder_variables];
        *value += (*global_predicted_output)[i] * global_scores[i];
    }
