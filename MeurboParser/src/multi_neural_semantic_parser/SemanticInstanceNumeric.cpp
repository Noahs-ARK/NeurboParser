...
void SemanticInstanceNumeric::Initialize(
    ...
    SemanticOptions *options =
            static_cast<NeuralSemanticPipe *>(dictionary.GetPipe())->GetSemanticOptions();
    ...
    for (int k = 0; k < instance->GetNumPredicates(); k++) {
        ...
        for (int l = 0; l < num_arguments; ++l) {
            ...
            if (id < 0) {
                ...
                //LOG(INFO)<< "Encountered unk" << id<<endl;
            }
            CHECK_GE(id, 0);
...
void SemanticInstanceNumeric::ComputeDependencyInformation(
    ...
    // Select task2sive/active voice.
    is_task2sive_voice_.assign(instance_length, false);
    for (int i = 0; i < instance_length; ++i) {
        is_task2sive_voice_[i] = ComputePassiveVoice(instance, i);
    }
