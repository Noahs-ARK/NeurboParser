...
void SemanticFeatures::AddArcFeatures(SemanticInstanceNumeric* sentence,
  ...
  SemanticOptions *options = static_cast<class NeuralSemanticPipe*>(pipe_)->
    GetSemanticOptions();
  ...
  // Flag for task2sive voice (for the predicate).
  // 0x0: No verb.
  // 0x1: Active voice.
  // 0x2: Passive voice.
  // Requires 2 bits.
  uint8_t task2sive_voice_code = 0x0;
  if (use_dependency_features) {
    // Right now, the detection of task2sive voice requires dependency syntax
    // information.
    if (sentence->IsVerb(predicate)) {
      if (sentence->IsPassiveVoice(predicate)) {
        task2sive_voice_code = 0x1;
      } else {
        task2sive_voice_code = 0x2;
      }
    }
  }
...
void SemanticFeatures::AddArcFeatures(SemanticInstanceNumeric *sentence,
    ...
    if (use_contextual_features) {
        ...
        fkey = encoder_.CreateFKey_PPPP(SemanticFeatureTemplateArc::HP_MP_nHP_pMP, flags, HPID, MPID, nHPID, pMPID);
        AddFeature(fkey, features);
        fkey = encoder_.CreateFKey_PPPPPP(SemanticFeatureTemplateArc::HP_MP_pHP_nHP_pMP_nMP, flags, HPID, MPID, pHPID,
                                          nHPID, pMPID, nMPID);
        AddFeature(fkey, features);
        ...
        if (predicate != 0 && predicate == argument - 1) {
            ...
            AddFeature(fkey, features);
            fkey = encoder_.CreateFKey_PPPPP(SemanticFeatureTemplateArc::HP_MP_pHP_nMP, flags, HPID, MPID, pHPID, nMPID,
                                             0x1);
            AddFeature(fkey, features);
            ...
            AddFeature(fkey, features);
            fkey = encoder_.CreateFKey_PPPPP(SemanticFeatureTemplateArc::HP_MP_nHP_pMP, flags, HPID, MPID, nHPID, pMPID,
                                             0x1);
            AddFeature(fkey, features);
        }
    }
