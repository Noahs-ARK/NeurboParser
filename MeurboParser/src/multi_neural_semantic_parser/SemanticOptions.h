...
public:
    ...
    int pre_word_dim () { return pre_word_dim_; }
    ...
    const string GetTestFilePath(const string & formalism) {
        if (formalism == "task1") {
            return task1_file_test_;
        }
        else if (formalism == "task2") {
            return task2_file_test_;
        }
        else if (formalism == "task3") {
            return task3_file_test_;
        }
        else {
            LOG(INFO) <<"Unsupported formalism: " <<formalism <<". Giving up..."<<endl;
            CHECK(1 == 0);
    ...
    const string GetOutputFilePath(const string & formalism) {
        if (formalism == "task1") {
            return task1_file_prediction_;
        }
        else if (formalism == "task2") {
            return task2_file_prediction_;
        }
        else if (formalism == "task3") {
            return task3_file_prediction_;
        }
        else {
            LOG(INFO) <<"Unsupported formalism: " <<formalism <<". Giving up..."<<endl;
            CHECK(1 == 0);
    ...
    void SetFilePath(const string &formalism) {
        if (formalism == "task1") {
            file_train_ = task1_file_train_;
            file_test_ = task1_file_test_;
            file_prediction_ = task1_file_prediction_;
        }
        else if (formalism == "task2") {
            file_train_ = task2_file_train_;
            file_test_ = task2_file_test_;
            file_prediction_ = task2_file_prediction_;
        }
        else if (formalism == "task3") {
            file_train_ = task3_file_train_;
            file_test_ = task3_file_test_;
            file_prediction_ = task3_file_prediction_;
        }
        else {
            LOG(INFO) <<"Unsupported formalism: " <<formalism <<". Giving up..."<<endl;
            CHECK(false);
    ...
    const string &GetPrunerModelFilePath(const string &formalism) {
        if (formalism == "task1") {
            return task1_file_pruner_model_;
        } else if (formalism == "task2") {
            return task2_file_pruner_model_;
        } else if (formalism == "task3") {
            return task3_file_pruner_model_;
        } else {
            LOG(INFO) << "Unsupported formalism: " << formalism << ". Giving up..." << endl;
            CHECK(1 == 0);
        }
...
protected:
    string task1_file_train_;
    string task1_file_test_;
    string task1_file_prediction_;
    string task1_file_pruner_model_;
    string task2_file_train_;
    string task2_file_test_;
    string task2_file_prediction_;
    string task2_file_pruner_model_;
    string task3_file_train_;
    string task3_file_test_;
    string task3_file_prediction_;
    string task3_file_pruner_model_;
